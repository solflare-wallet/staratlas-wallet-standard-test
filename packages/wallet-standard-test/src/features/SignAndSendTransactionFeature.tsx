...
export default ((props) => {
  ...
  const feature = () => props.wallet.features[SolanaSignAndSendTransaction];
  const accountsWithSignAndSendTransaction = createMemo(() =>
  ...
  const onSignTransaction = (
    ...
    const accounts = accountsWithSignAndSendTransaction().filter(
    ...
    toast.promise(
      connection()
        ...
        .then((rbh) => {
          ...
          return feature().signAndSendTransaction(
        ...
        }),
      {
        ...
        success: `Transaction${accounts.length === 1 ? '' : 's'} signed successfully. Signature confirmation not yet implemented.`,
        error: (e) => {
  ...
  return (
    <div class="grid grid-cols-1 items-center justify-between gap-1">
      ...
      <Dialog>
        <DialogTrigger
          as={Button<'button'>}
          disabled={accountsWithSignAndSendTransaction().length === 0}
        >
        ...
        <DialogContent class="bg-transparent backdrop-blur-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Sign and send a transaction sending lamports. All listed accounts
              should successfully sign and send. Only mainnet and legacy
          ...
          <SelectWalletComboBox
            ...
            accounts={accountsWithSignAndSendTransaction()}
...
}) satisfies Component<FeatureProps<SolanaSignAndSendTransactionFeature>>;
